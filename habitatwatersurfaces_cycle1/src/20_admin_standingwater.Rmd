...
coord_sample_replaced <- mhq_watersurfaces_refpoints_orig %>%
  ...
  select(-grts_ranking, -code_watersurfaces) %>%
...
This way we get only one referece point for each watersurface.
...
mhq_refpoints_double <- mhq_refpoints_double %>%
  mutate(grts_ranking = (terra::extract(grts_master, mhq_refpoints_double))$GRTSmaster_habitats) %>%
  select(pol_id, code_watersurfaces, grts_ranking)
