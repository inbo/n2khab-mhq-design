...
+ in the file OverzichtMeetnet&Steekproef_HT31XX_18_12_2023.xlsx for types 31xx
...
```{r}
admin_orig <- read_excel("../data/OverzichtMeetnet&Steekproef_HT31XX_18_12_2023.xlsx") %>%
  mutate(ID_Plas = str_remove(ID_Plas, "\\.0"))
...
check <- length(unique(mhq_watersurfaces_pol$pol_id)) == nrow(mhq_watersurfaces_pol)
...
mhq_watersurfaces_refpoints_extra <- mhq_watersurfaces_refpoints_extra %>%
  mutate(grts_ranking_extra = (terra::extract(grts_master,mhq_watersurfaces_refpoints_extra))$GRTSmaster_habitats) 
...
mhq_watersurfaces_refpoints <- mhq_watersurfaces_refpoints_orig %>%
  bind_rows(select(mhq_watersurfaces_refpoints_extra, pol_id, grts_ranking = grts_ranking_extra, code_watersurfaces))
```
...
unique_match <- mhq_watersurfaces_samplingunits2 %>%
  filter(!is.na(code_watersurfaces)) %>%
  st_drop_geometry() %>%
  distinct(code_watersurfaces, pol_id) %>%
  group_by(code_watersurfaces) %>%
  mutate(n_pol = n_distinct(pol_id)) %>%
  ungroup() %>%
  group_by(pol_id) %>%
  mutate(n_ws = n_distinct(code_watersurfaces)) %>%
  ungroup() %>%
  filter(n_pol == 1 & n_ws == 1) %>%
  rename(add_ws_code = code_watersurfaces)

mhq_watersurfaces_refpoints <- mhq_watersurfaces_refpoints %>%
  left_join(unique_match, by = "pol_id") %>%
  mutate(code_watersurfaces = ifelse(is.na(code_watersurfaces), add_ws_code, code_watersurfaces)) %>%
  select(-n_pol, -n_ws, -add_ws_code)
```
...
add_name <- check_measured_31xx %>%
  filter(n_match_code > 0)

add_ws <- assessments_mhq_watersurfaces %>%
  distinct(pol_id, code_watersurfaces) %>%
  filter(pol_id %in% add_name$pol_id) %>%
  rename(code_watersurfaces_add = code_watersurfaces)

mhq_watersurfaces_refpoints <- mhq_watersurfaces_refpoints %>%
  left_join(add_ws, by = "pol_id") %>%
  mutate(code_watersurfaces = ifelse(is.na(code_watersurfaces), code_watersurfaces_add, code_watersurfaces)) %>%
  select(-code_watersurfaces_add)
```
...
extra_replacement_refpoints <- extra_replacement_refpoints %>%
  mutate(grts_ranking_extra = (terra::extract(grts_master, extra_replacement_refpoints))$GRTSmaster_habitats,
...
mhq_watersurfaces_refpoints <- mhq_watersurfaces_refpoints %>%
  bind_rows(select(extra_replacement_refpoints, pol_id, grts_ranking = grts_ranking_extra, code_watersurfaces))
...
watersurfaces_no_target_type_refpoints <- watersurfaces_no_target_type_refpoints %>%
  mutate(grts_ranking = (terra::extract(grts_master$GRTSmaster_habitats, watersurfaces_no_target_type_refpoints))$GRTSmaster_habitats) %>%
...
watersurfaces_not_assessed_overlap_refpoints <- watersurfaces_not_assessed_overlap_refpoints %>%
  mutate(grts_ranking_new = (terra::extract(grts_master, watersurfaces_not_assessed_overlap_refpoints))$GRTSmaster_habitats) %>%
...
In this aggregation we set grts_ranking and grts_ranking_draw to the minimum value of both original sampling units.
After aggregation we should only have one reference point for the watersurface.
Therefore we only keep the reference point of the original sampling unit with the lowest grts_ranking (and we remove the reference point of the other original sampling unit within the same watersurface). 
This way we get only one reference point for each watersurface.
...
assessments_mhq_watersurfaces_corr <- assessments_mhq_watersurfaces %>%
  filter(assessment) %>%
  select(-pol_id, -type_target_all, -is_type_target, -any_type_target, -grts_ranking, -grts_ranking_draw, -area_class_all, -note) %>%
  distinct()
...
assessments_mhq_watersurfaces_double2 <- assessments_mhq_watersurfaces %>%
  ...
  summarise(type_target_all = str_c(unique(type_target_all), collapse = ";"),
            ...
            area_class_all = str_c(unique(area_class_all), collapse = ";"),
...
check_double3 <- assessments_mhq_watersurfaces_double3 %>%
...
mhq_refpoints_double <-  assessments_mhq_watersurfaces_double3 %>%
  distinct(pol_id, code_watersurfaces) %>%
  separate_rows(pol_id, sep = ";") %>%
  left_join(select(mhq_watersurfaces_refpoints, -code_watersurfaces), by = "pol_id")

mhq_refpoints_double_min_ranking <- mhq_refpoints_double %>%
  group_by(code_watersurfaces) %>%
  filter(rank(grts_ranking, ties.method = "random") == 1) %>%
  ungroup()
...
mhq_watersurfaces_refpoints <- mhq_watersurfaces_refpoints %>%
  filter(!pol_id %in% mhq_refpoints_double$pol_id) %>%
  bind_rows(mhq_refpoints_double_min_ranking)

```

```{r}
no_ws_code <-mhq_watersurfaces_refpoints %>%
  filter(is.na(code_watersurfaces)) %>%
  select(-code_watersurfaces) %>%
  st_join(watersurfaces) %>%
  left_join(select(assessments_mhq_watersurfaces, pol_id, assessment, measurement_biotic, measurement_abiotic))
```
...
mhq_watersurfaces_objects <- assessments_mhq_watersurfaces %>%
  ...
  st_as_sf() %>%
  select(-area_ws) %>%
  filter(grts_ranking != 34352082) #manual correction
...
check_wc_code <- mhq_watersurfaces_refpoints3 %>%
  anti_join(st_drop_geometry(watersurfaces), by = "code_watersurfaces")
...
+ watersurfaces_refpoints (git2rdata object): defines the reference point for the watersurfaces in the original sampling frame and grts-ranking for this refpoint
...
+ watersurfaces_assessments: table with assessment for the monitoring habitat quality programme
  ...
  + any_habitat: 
    ...
    + `NA` when the evaluated type is absent or unknown and it is not known if any other type is present (recorded as `doelhabitat` = `nee` or in case a watersurface is inaccessible)
...
+ mhq_watersurfaces_populationunits: table with information on mhq population units from which sampling units are drawn
  + sampling_unit_code: code for the sampling unit, we take the current polygon_id as the sampling_unit_code 
  + polygon_id
...
mhq_watersurfaces_populationunits_typetarget <- assessments_mhq_watersurfaces %>%
  ...
  select( pol_id, code_watersurfaces, type, grts_ranking, grts_ranking_draw) %>%
...
mhq_watersurfaces_populationunits <- mhq_watersurfaces_populationunits %>%
  ...
  group_by(polygon_id, type, grts_ranking) %>%
  ...
  ungroup() %>%
  mutate(sampling_unit_code = polygon_id) %>%
  select(sampling_unit_code, polygon_id, everything()) %>%
  filter(!is.na(polygon_id)) %>%
  arrange(type, grts_ranking_draw, polygon_id)

check <- mhq_watersurfaces_populationunits %>%
  group_by(sampling_unit_code, type) %>%
  mutate(n = n()) %>%
  ungroup() %>%
  filter(n > 1)
