...
```{r}
calc_finitsamplesize_terrhab <- function(n_type_flanders = 80, n_type_sac = 170, n_subtype_flanders = 80, n_polygons_min = 15, area_min = 10, types_area_strata = types_area_strata, exclude_type = c("9110", "1130", "1140", "1310", "5130", "2110", "6120")) {
...
meetnet_selection <- samplesize_original_detailed %>%
  distinct(type, main_type, typelevel, sample_type, extra_subtype) %>%
  ...
  select(typeclass_name, main_type, main_type_name, sample_type, subtype, subtype_name, extra_subtype)

```

Tabel \@ref(tab:tabschemes) geeft een overzicht van de habitattypen waarvoor een meetnet bestaat en de habitatsubtypen waarvoor een overbemonstering gebeurt. 

```{r tabschemes}
meetnet_selection %>%
  select(habitattype = main_type_name, "code habitattype" = main_type, meetnet = sample_type, habitatsubtype = subtype_name, "code habitatsubtype" = subtype, "overbemonstering habitatsubtype" = extra_subtype) %>%
  kable() %>%
  kable_styling() %>%
...
Habitattype 7230 voldoet aan de criteria voor meetnet, maar nog niet operationeel. Hiervoor meetnet?
