...
check_double <- assessments_mhq_watersurfaces_double %>%
  group_by(pol_id, type_observed) %>%
  mutate(n = n()) %>%
  ungroup() %>%
  filter(n > 1)
...
check_geom <- mhq_watersurfaces_refpoints3 %>%
  filter(!st_is(mhq_watersurfaces_refpoints3$geometry, "POINT"))
...
watersurfaces_refpoints <- mhq_watersurfaces_refpoints3 %>%
  st_drop_geometry() %>%
  mutate(x = st_coordinates(mhq_watersurfaces_refpoints3)[,1],
         y = st_coordinates(mhq_watersurfaces_refpoints3)[,2]) %>%
...
+ watersurfaces_assessments: table with assessment for the monitoring habitat quality programme
  + code_watersurfaces
  + type_target
  + type_observed_all: all observed habitat types (a watersurface can contain more than one habitat type)
  + assessment_source: field assessment or orthophoto
  + assessment_date
  + inaccessible: long term or short term
  + is_type_target: does the target type correspond with (one of) the observed type(s)

```{r}
mhq_watersurfaces_overview <- assessments_mhq_watersurfaces %>%
  filter(!is.na(assessment)) %>% 
  mutate(year_biotic = ifelse(year_biotic == 0, NA, year_biotic),
         assessment_year = pmin(year_abiotic, year_abiotic, na.rm = TRUE),
  ...
  select(pol_id, code_watersurfaces, type_target_all, type_observed, assessment_source, assessment_date, inaccessible, is_type_target, measurement_biotic, measurement_abiotic, measurement_biotic_year = year_biotic, measurement_abiotic_year = year_abiotic, db_ref = code_inboveg)
...
mhq_watersurfaces_assessments <- mhq_watersurfaces_overview %>%
  select(code_watersurfaces, type_target_all, type_observed, assessment_source, assessment_date, inaccessible, is_type_target) %>%
  ...
  replace_na(list(is_type_target = FALSE, type_observed_all = "gh")) %>%
  filter(!(is.na(code_watersurfaces) & !is_type_target))

mhq_watersurfaces_assessments %>%
  write_vc(file = "mhq_watersurfaces_assessments", root = "../output", sorting = c("code_watersurfaces", "type_target"), strict = FALSE)
...
+ mhq_watersurfaces_measurements: table with information on measurements
  ...
  + type_observed
  + measurement_biotic: biotic measurement performed?
  + measurement_abiotic: abiotic measurement performed?
  + measurement_biotic_year: year of biotic measurement
  + measurement_abiotic_year: year of abiotic measurement


```{r}
mhq_watersurfaces_measurements <- mhq_watersurfaces_overview %>%
  filter(measurement_abiotic | measurement_abiotic) %>%
  select(code_watersurfaces, db_ref , type_observed, measurement_biotic, measurement_abiotic, measurement_biotic_year, measurement_abiotic_year) 

mhq_watersurfaces_measurements %>%
  write_vc(file = "mhq_watersurfaces_measurements", root = "../output", sorting = c("code_watersurfaces", "type_observed"), strict = FALSE)
```
...
+ mhq_watersurfaces_samplingunits: table with information on mhq sampling untis
  + scheme: name of the monitoring scheme
  + sampling_unit_code: code for the sampling unit consisting of the watersurface code and the target type (necessary?; sampling unit always corresponds to the full watersurface, but a watersurface can contain different target type; a watersurface can belong to different schemes)
  + code_watersurfaces
  ...
  + source_samplingunit: (not strictly necessary ?) distinguishes between 
...
schemes_mhq <- read_scheme_types() %>%
    ...
    select(scheme, type_target = type)

mhq_watersurfaces_samplingunits_orig <- assessments_mhq_watersurfaces %>%
  distinct(code_watersurfaces, grts_ranking, grts_ranking_draw, type_target_all) %>%
  separate_rows(type_target_all, sep =";") %>%
  rename(type_target = type_target_all) %>%
  mutate(type_target = ifelse(type_target == "2190", "2190_a", type_target)) %>%
  left_join(schemes_mhq, by = "type_target") %>%
  select(scheme, code_watersurfaces, type_target, grts_ranking, grts_ranking_draw) %>%
  filter(!is.na(code_watersurfaces)) %>%
  filter(!is.na(type_target)) %>%
  mutate(source_samplingunit = "original sample frame")

mhq_watersurfaces_samplingunits_new <- assessments_mhq_watersurfaces %>%
  filter(measurement_biotic | measurement_abiotic) %>%
  filter(!is_type_target) %>%
  filter(type_observed %in% mhq_watersurfaces_samplingunits_orig$type_target) %>%
  mutate(type_target = type_observed,
         grts_ranking_draw = ifelse(is.na(grts_ranking_draw), grts_ranking, grts_ranking_draw)) %>%
  left_join(schemes_mhq, by = "type_target") %>%
  select(scheme, code_watersurfaces, type_target, grts_ranking, grts_ranking_draw) %>%
  mutate(source_samplingunit = "assessments monitoring cycle 1")

mhq_watersurfaces_samplingunits <- mhq_watersurfaces_samplingunits_orig %>%
  bind_rows(mhq_watersurfaces_samplingunits_new) %>%
  mutate(scheme = ifelse(type_target == "2190_a", "HQ2190_aq", as.character(scheme))) %>%
  group_by(scheme, code_watersurfaces, type_target, grts_ranking) %>%
  summarise(grts_ranking_draw = min(grts_ranking_draw),
            source_samplingunit = ifelse(any(source_samplingunit == "original sample frame"),
                                             "original sample frame", unique(source_samplingunit))) %>%
  ungroup() %>%
  mutate(sampling_unit_code = str_c(code_watersurfaces, type_target, sep = "_")) %>%
  select(scheme, sampling_unit_code, code_watersurfaces, everything())

check <- mhq_watersurfaces_samplingunits %>%
...
mhq_watersurfaces_samplingunits %>%
  write_vc(file = "mhq_watersurfaces_samplingunits", root = "../output", sorting = c("scheme", "sampling_unit_code"), strict = FALSE)
```
