...
+ watersurfaces_assessments: table with assessment for the monitoring habitat quality programme
  ...
  + is_habitat: 
    + `TRUE` if a habitat type was observed
    + `FALSE` if no habitat type was observed, altough a regional import biotope (rib) type might be present (the presence of rib types was not evaluated)
    + `NA` if the type could not be determined 
...
mhq_watersurfaces_overview <- mhq_watersurfaces_overview %>%
  ...
  mutate(type_observed = ifelse(type_observed == "2190", "2190_a", type_observed),
         ...
         is_habitat = !is.na(type),
         is_habitat = ifelse(type_observed == "unknown" & !is.na(type_observed), NA, is_habitat))


mhq_watersurfaces_assessments <- mhq_watersurfaces_overview  %>%
  select(assessment_date, polygon_id = code_watersurfaces, type, is_habitat, assessment_source, inaccessible) %>%
...
mhq_watersurfaces_measurements <- mhq_watersurfaces_overview %>%
  ...
  select(code_watersurfaces, db_ref , type, is_habitat, type_observed, measured_biotic = measurement_biotic, measured_abiotic = measurement_abiotic, year_biotic = measurement_biotic_year, year_abiotic = measurement_abiotic_year) %>%
  ...
  select(measurement_date, polygon_id = code_watersurfaces, db_ref, measurement_type, type, is_habitat) %>%
...
mhq_watersurfaces_populationunits <- mhq_watersurfaces_populationunits %>%
  ...
  mutate(scheme = ifelse(type == "2190_a", "HQ2190_aq", as.character(scheme))) %>%
...
mhq_watersurfaces_populationunits %>%
  write_vc(file = "mhq_watersurfaces_populationunits", root = "../output", sorting = c("scheme", "sampling_unit_code"), strict = FALSE)
```
