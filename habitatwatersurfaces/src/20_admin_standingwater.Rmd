...
+ mhq_watersurfaces_assessments: table with assessment for the monitoring habitat quality programme
  + code_watersurfaces
  + type_target
...
mhq_watersurfaces_assessments <- mhq_watersurfaces_overview %>%
  ...
  mutate(is_type_target = str_detect(string = type_observed_all, pattern =  type_target),
         is_type_target = ifelse(is.na(is_type_target), FALSE, is_type_target)) %>%
...
mhq_watersurfaces_samplingunits <- mhq_watersurfaces_samplingunits_orig %>%
  ...
  mutate(scheme = ifelse(type_target == "2190_a", "HQ2190_aq", scheme)) %>%
...
check <- mhq_watersurfaces_samplingunits %>%
  group_by(scheme, code_watersurfaces, type_target) %>%
  mutate(n = n()) %>%
  ungroup() %>%
  filter(n > 1)

mhq_watersurfaces_samplingunits %>%
  write_vc(file = "mhq_watersurfaces_samplingunits", root = "../output", sorting = c("scheme", "code_watersurfaces", "type_target"), strict = FALSE)
```
