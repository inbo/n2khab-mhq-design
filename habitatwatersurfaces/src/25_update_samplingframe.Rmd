...
watersurfaces_populationunits <- mhq_watersurfaces_populationunits_update %>%
  left_join(mhq_type_observed, by = "polygon_id") %>%
  mutate(contains_type = type_observed == type,
         assessed = ifelse(is.na(assessed), FALSE, assessed)) %>%
  select(scheme, sampling_unit_code, polygon_id, type, type_observed, grts_ranking, grts_ranking_draw, source, assessed, contains_type)
...
watersurfaces_populationunits_select <- watersurfaces_populationunits %>%
  ...
  select(-assessed, -contains_type, -type_observed) %>%
  arrange(scheme, sampling_unit_code) %>%
  unique()

remove <- watersurfaces_populationunits_select %>%
  filter(source == "original sample frame") %>%
  anti_join(watersurfaces_hab_types, by = c("polygon_id", "type"))

watersurfaces_populationunits_select <- watersurfaces_populationunits_select %>%
