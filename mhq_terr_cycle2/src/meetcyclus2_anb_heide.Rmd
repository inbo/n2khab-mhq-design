...
# Vervangpunten voor heidehabitats
...
vervangpunten_anb <- vervangpunten_anb %>%
  ...
  st_as_sf(coords =c("x", "y"), crs = 31370) %>%
  st_transform(4326) %>%
  group_by(point_code_vervang) %>%
  filter(rank_rel_buffer <= 15 | is.na(rank_rel_buffer)) %>%
  ungroup()
...
tabel <- mhq_terr_measurements_anb %>%
  filter(str_sub(type, 1, 4) %in% heide) %>%
