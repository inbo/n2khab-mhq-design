...
Voor kleine polygonen zal dit maar een beperkt aantal vervangpunten opleveren. Daarom selecteren we voor polygonen met minder dan 50 steekproefeenheden en minder dan 10 seteekproefeenheden in de vierante buffer, alle steekproefeenheden als bijkomende potentiële vervangpunten. 
...
vervangpunten <- grts_habmap %>%
  mutate(grts_ranking_draw = grts_ranking) %>%
  semi_join(sampling_units_panels, by = "polygon_id") %>%
  ...
  ungroup() %>%
  filter((polygon_id == polygon_id_vervang) | (n_buffer <= 10 & n_polygon < 50) ) %>%
  group_by(polygon_id) %>%
...
vervangpunten %>%
  filter(polygon_id == p_id) %>%
  st_transform(4326) %>%
  mutate(show_color = ifelse(is.na(point_code_vervang), "blue", "yellow")) %>%
  leaflet() %>%
  addTiles() %>%
...
bscols(
  list(
    ...
    sd %>%
      ...
      addTiles() %>%
      addPolygons(data = polygon ) %>%
      addCircleMarkers(data = sampling_unit, color = "red", label = ~str_c(point_code, " - panel =", panel)) %>%
