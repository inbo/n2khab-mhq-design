...
sampleframe_3260_ranking_measured <- sampleframe_3260_ranking %>%
  filter(segment_id %in% all_measured_su$segment_id) %>%
  left_join(mhq_streams_refpoints, by = "point_code") %>%
...
segments_ranking <- sampleframe_3260_ranking_measured %>%
  ...
  bind_rows(watercourse_100mseg_points_grts%>%
              rename(geometry = geom)) %>%
  select(rank, vhag_code, prop_3260, sac_streams, segment_id, grts_ranking_draw) %>%
  mutate(segment_id = ifelse(is.na(segment_id), str_c("2023_", rank), segment_id),
...
segments_ranking %>%
  st_write("../output/100msegments_grts_ranking.gpkg", delete_layer = TRUE)
```
