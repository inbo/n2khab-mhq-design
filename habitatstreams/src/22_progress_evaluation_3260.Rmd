...
sample_assessments_3260_check <- sample_assessments_3260 %>%
  filter(is.na(assessment_date) | (is_type_target & segment_measurable)) %>%
...
## Selectie af te werken meetpunten op basis van steekproefgrootte versie 2021
...
sample_size_to_do <- progress_3260 %>%
  mutate(n_to_do = pmax(0, n_finite - n_target_type),
         n_visits = ceiling(n_to_do/detection_rate_pct * 100) * 1.5) %>%
  select( sac_streams, n_to_do, n_visits)
...
sample_units_to_do <- sample_to_do %>%
  filter(to_do) %>%
  select(sampling_unit_code, sac_streams,  grts_ranking, volgorde, x, y) %>%
  arrange(volgorde)

sample_units_to_do_sf <- sample_units_to_do %>%
...
write_csv2(sample_units_to_do, "../output/meetpunten3260_veldwerk2021_versie2021-05-21.csv")
