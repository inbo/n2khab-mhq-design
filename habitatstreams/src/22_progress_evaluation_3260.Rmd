...
Onderstaande tabel geeft een overzicht van de opgemeten steekproefeenheden die niet in de steekproef versie 1.7 (volgens het steekproefkader versie 1.7) zitten. Drie daarvan zitten wel in de steekproef versie 1.4 (volgens het steekproefkader versie 1.7). Deze zitten niet meer in de laatste versie de steekproef omdat het steekproefkader is gewijzigd. Twee punten zitten enkel in de steekproef die getrokken werd op basis van het rapport (Westra et al., 2014). Hierin gebeurde er een fout bij de verdeling van de gewenste steekproefgrootte over de strata. Deze fout is nu rechtgezet. De impact van deze fout is dus relatief beperkt. 

Omdat de steekproefeenheden correct werden opgemeten volgens de toen geldende instructies, zullen we deze opnemen in de steekproef versie 1.7. We geven dit aan door te werken met een variabele 'grts_ranking_draw' die de grts_ranking weergeeft op basis waarvan de steekproeftrekking gebeurt. We geven deze punten dan een zeer lage waarde voor grts_ranking_draw zodat ze met zekerheid opgenomen worden in de steekproef. Zie verder.

We moeten er wel voor waken dat de steekproef voldoende representatief blijft voor de huidige doelpopulatie. Maar we stellen voor om minstens twee opmetingen te doen per steekproefeenheid vooraleer verouderde steekproefeenheden (die niet meer geselecteerd zouden worden volgens de laatste versie van het steekproefkader) te vervangen.
...
sample_assessments_3260_2 <- sample_assessments_3260 %>%
  ...
  mutate(grts_ranking_draw = ifelse(sampling_unit_code %in% sample_assessments_3260_not_valid$sampling_unit_code, rank(grts_ranking), grts_ranking),
         type_target = "3260",
         scheme = "MHQ3260",
...
sample_assessments_3260_update <- sample_assessments_3260_original %>%
  ...
  mutate(sampling_unit_code = str_c(sampling_unit_code, "_3260"))
...
De tabel 'mhq_streams_refpoints' geeft een overzicht van de zogenaamde referentiepunten voor alle steekproefeenheden in het meetnet voor habitattype 3260. Elke steekproefeenheid heeft een referentiepunt waar naar toe genavigeerd wordt om de meting uit te voeren. Bij stromende wateren is het referentiepunt het startpunt van het 100 meter segment (en meer specifiek het meest stroomopwaarts gelegen punt van het segment).
...
mhq_streams_refpoints <- sample_assessments_3260_update %>%
  mutate(legacy_site = FALSE,
         db_ref = field_code) %>%
  select(scheme, sampling_unit_code, point_code, type_target, sac_streams, grts_ranking, grts_ranking_draw, legacy_site, x, y, db_ref) %>%
  arrange(grts_ranking_draw, point_code)

write_vc(mhq_streams_refpoints, file = "mhq_streams_refpoints", root = "../output", sorting = c("sampling_unit_code", "point_code"))
```
...
mhq_streams_assessments <- sample_assessments_3260_update %>%
  ...
  select(point_code, type_target, is_type_target, type_observed, inaccessible, measurable = segment_measurable, assessment_date, assessment_source, fieldwork_team, x, y) 

write_vc(mhq_streams_assessments, file = "mhq_streams_assessments", root = "../output", sorting = c("point_code"))
```
