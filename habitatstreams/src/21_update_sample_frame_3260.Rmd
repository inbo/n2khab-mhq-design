...
streams_segments_no_ranking <- vha_segments_combined %>%
...
overview_missing_ranking <-  vha_segments_combined %>%
    rename(river_name = name) %>%
  st_as_sf(coords = c("x", "y"), crs = 31370) %>%
  st_join(flanders) %>%
  st_drop_geometry() %>%
...
grts_border <- grts_master %>%
  mask(st_buffer(streams_segments_no_ranking, 32)) %>%
  rasterToPolygons() %>%
  st_as_sf()
...
segments_3260_border_grts <- segments_3260_border %>%
 st_join(grts_border, join = st_nearest_feature) %>%
  ...
  filter(!is.na(name) | river_name == "Maas") %>%
  select(-name)
