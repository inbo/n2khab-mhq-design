...
```
# Manual check and conclusion per plot
...
We combine this information with (1) the plots with a type match between the vbi assessment and the habitatmap and (2) the plots for which the type has not been assessed in the field (and can only be derived based on the habitatmap).
This way we get an overview of the types per plot in the forest inventory, based on the best available information.

```{r}

manual_check <- read_csv2("../data/check_vbi_habitatmap_controle.csv")

manual_check <- manual_check %>%
  select(plot_id, periode, type_vbi = type_obser, type_habmap = type_habit, type_control = Verdict, note_manual_check = Reason) %>%
  filter(!is.na(type_control)) %>%
  mutate(type_control = ifelse(type_control == "91E0vn (of gh)", "91E0_vn", type_control),
  ...
  select(plot_id, periode, type, source, note_manual_check)
...
type_match <- vbi_meetpunten_hab_observed_rule1 %>%
  ...
  select(plot_id, periode, type = type_decided, phab, source)
```

```{r}
type_not_assessed <- vbi_meetpunten_hab %>%
  filter(is.na(type_observed)) %>%
  filter(str_sub(type, 1, 2) == "91") %>%
  select(plot_id, periode, type, phab) %>%
  mutate(source = "habitatmap2023")

type_vbi_conclusion <- manual_check %>%
  bind_rows(type_match) %>%
  bind_rows(type_not_assessed) %>%
  arrange(plot_id) %>%
  filter(str_sub(type, 1, 2) == "91")

write_csv2(type_vbi_conclusion, "../output/type_observed_vbi.csv", na = "")
```
...
We assign a grts_ranking to all population units by overlay the habitatmap with the grts_master (containing the grts-ranking).
...
```{r}
sample_extra <- read_csv2("../output/mhq_forests_cycle1.csv")

n_extra_cycle1 <- n_distinct(sample_extra$point_code)
...
n_measured <- sum(mhq_measured$n_measured_extra)

n_tot <- n_extra_cycle1 + n_measured

n_tot_year <- n_tot/12
```

```{r}
type_observed <- read_csv2("../output/type_observed_vbi.csv")

overzicht <- type_observed %>%
  mutate(type = ifelse(type == "91EO_va", "91E0_va", type),
         phab = ifelse(is.na(phab), 100, phab)) %>%
  group_by(type) %>%
  summarise(n_records = n(),
            n_expected = sum(phab/100)) %>%
  ungroup()
